create or replace function public.create_tag_bound_idempotent_private_pending_submission(
  p_client_submission_id uuid,
  p_expected_active_tag_id uuid,
  p_rider_name text,
...
begin
  ...
  if p_expected_active_tag_id is null then
    raise exception 'Expected active tag ID is required.';
  end if;
  ...
  if v_submission_id is not null then
    if v_active_tag_id <> p_expected_active_tag_id then
      raise exception 'IDEMPOTENT_SUBMISSION_TAG_MISMATCH';
    end if;
  ...
  if v_active_tag_id <> p_expected_active_tag_id then
    raise exception 'ACTIVE_TAG_CHANGED';
  end if;
...
revoke execute on function public.create_tag_bound_idempotent_private_pending_submission(
  uuid,
  uuid,
  text,
  text,
...
grant execute on function public.create_tag_bound_idempotent_private_pending_submission(
  uuid,
  uuid,
  text,
  text,

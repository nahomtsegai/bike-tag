...
add column if not exists found_latitude double precision,
add column if not exists found_longitude double precision,
add column if not exists found_location_accuracy_meters double precision,
add column if not exists found_location_captured_at timestamptz;
...
alter table public.submissions
add constraint submissions_found_latitude_range_check
check (
  found_latitude is null or
  found_latitude between -90 and 90
);

alter table public.submissions
add constraint submissions_found_longitude_range_check
check (
  found_longitude is null or
  found_longitude between -180 and 180
);

alter table public.submissions
add constraint submissions_found_location_accuracy_meters_check
check (
  found_location_accuracy_meters is null or
  found_location_accuracy_meters >= 0
);

alter table public.tags
add constraint tags_found_latitude_range_check
check (
  found_latitude is null or
  found_latitude between -90 and 90
);
...
alter table public.tags
add constraint tags_found_longitude_range_check
check (
  found_longitude is null or
  found_longitude between -180 and 180
);
...
alter table public.tags
add constraint tags_found_location_accuracy_meters_check
check (
  found_location_accuracy_meters is null or
  found_location_accuracy_meters >= 0
);
...
begin
  ...
  if p_found_latitude is not null and
    (p_found_latitude < -90 or p_found_latitude > 90)
  then
    raise exception 'Found latitude is invalid.';
  end if;

  if p_found_longitude is not null and
    (p_found_longitude < -180 or p_found_longitude > 180)
  then
    raise exception 'Found longitude is invalid.';
  end if;

  if p_found_location_accuracy_meters is not null and
    p_found_location_accuracy_meters < 0
  then
  ...
  return query
  select v_submission_id, v_active_tag_id;
end;
$$;
...
begin
  ...
  return query
  select v_submission.id, v_found_tag_id, v_current_tag_id;
end;
$$;
